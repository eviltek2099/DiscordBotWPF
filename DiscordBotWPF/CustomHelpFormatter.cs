...
namespace DiscordBotWPF
{
  ...
  {
    [Command("greet"), Aliases("hello", "hi"), Description("Warm welcome!")]
    ...
    /////////////////////////////////////////////// 
    [Command("say")]
    [Description("Used for the bot to echo a line")]
    public async Task SayAsync(CommandContext ctx, [Description("This is what to repeat")] params string[] args)
    {
      await ctx.Channel.SendMessageAsync(String.Join(" ", args)).ConfigureAwait(false);
    }


    [Command("kill"), Hidden]
    [Description("This is a secret command to kill....")]
    //[RequireRoles(RoleCheckMode.Any, "Role name", "Role 2")]
    public async Task KillJokeAsync(CommandContext ctx, [Description("Enter the name of the person you want to kill.")] string name = "")
    {
      bool result = false;

      foreach (var role in ctx.Member.Roles)
      {
        if (PermissionLevel.Allowed == role.CheckPermission(Permissions.ManageChannels))
        {
          result = true;
          break;
        }
      ...
      if (result)
      {
        if (name == string.Empty)
          await ctx.Channel.SendMessageAsync("Kill who?").ConfigureAwait(false);
        else
          await ctx.Channel.SendMessageAsync("But I like " + name + " :(").ConfigureAwait(false);
      }
      else
        await ctx.Channel.SendMessageAsync("Denied!").ConfigureAwait(false);
    }


    [Command("respondmessage")]
    //This waits and then repeats the message thats next sent
    public async Task ResponceMessageAsync(CommandContext ctx)
    {
    ...
    //This waits for a emoji to a message and response with that emoji
    public async Task ResponceReactionAsync(CommandContext ctx)
    {
      var interactivity = ctx.Client.GetInteractivity();
    ...
    [Command("join")]
    //This allows a user to join a role
    public async Task JoinAsync(CommandContext ctx)
    {
      //var test = new DiscordEmbedBuilder.EmbedThumbnail();
      //test.Url = ctx.Client.CurrentUser.AvatarUrl;
      ...
      var role = ctx.Guild.GetRole(905164748998475796); //811740558048886864
      ...
      if (result.Result.Emoji == thumbsUpEmoji)
        ...
        await ctx.Channel.SendMessageAsync("Nothing changed").ConfigureAwait(false);
    ...
    [Command("poll")]
    public async Task PollAsync(CommandContext ctx, TimeSpan duration, params DiscordEmoji[] emojiOptions)
    {
      var interactivity = ctx.Client.GetInteractivity();
      ...
      //await interactivity.DoPollAsync(pollMessage, emojiOptions);
      ...
      //var results = result.Reactions.Select(x => $"{x.Emoji}: {x.Total}");
    ...
    [Command("oldpoll"), Description("Run a poll with reactions.")]
    ...
    //  await interactivity.SendPaginatedMessageAsync(ctx.Member, lipsum_pages);
    //}
  ...
  public class CustomHelpFormatter2 : DefaultHelpFormatter
  {
    public CustomHelpFormatter2(CommandContext ctx) : base(ctx) { }
  ...
  public class Handlers
  {
    ...
    {
     // if (e.Guild?.Id == 606289077263794238 && e.Author.Id == 127081250031665152)
     //   await e.Message.CreateReactionAsync(DiscordEmoji.FromName(s, ":ok_hand:"));
      if (e.Message.Content.ToLower().StartsWith("ping"))
        await e.Message.RespondAsync("pong! 😀");

      if (e.Message.Content.ToLower().StartsWith("ask"))
        await DropDown(e.Message.Channel);
    }
    public static async Task MessageUpdated(DiscordClient s, Message​Update​Event​Args e)
    {
      if (e.Guild?.Id == 606289077263794238 && e.Author.Id == 127081250031665152)
        await e.Message.DeleteAsync();
    }
    public static Task MemberAdded(DiscordClient s, GuildMemberAddEventArgs e)
    {
      // Non asynchronous code here.
      return Task.CompletedTask;
    }
    public static async Task ComponentInteraction(DiscordClient s, ComponentInteractionCreateEventArgs e) {

   //   var member = ctx.Member;
   //   await ctx.Guild.GrantRoleAsync(member, role,"They Asked");


      await e.Interaction.CreateResponseAsync(InteractionResponseType.UpdateMessage, 
        new DiscordInteractionResponseBuilder().WithContent("No more buttons for you >:)"));
    }
    //WaitForButtonAsync for WaitForSelectAsync
    ...
    public static async Task DropDown(DiscordChannel c)
    {
      ...
      var builder = new DiscordMessageBuilder().WithContent("Look, it's a dropdown!").AddComponents(myButton).AddComponents(dropdown);
      
      await builder.SendAsync(c); // Replace with any method of getting a channel. //
